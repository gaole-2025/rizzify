...
import { copyFile, deleteFile, R2_BUCKETS } from "@/lib/storage";
...
export class MockWorker {
  ...
  async start(): Promise<void> {
    ...
    try {
      ...
      try {
        ...
        console.error("❌ Failed to register job handlers:", handlerError);
        throw handlerError;
      }
  ...
  private async registerJobHandlers(): Promise<void> {
    ...
    // 注册图片生成任务处理器
    // pg-boss work() 方法签名: work(queueName, handler, options)
    this.boss.work(
      "task_generate",
      async (job: any) => {
   ...
   * 处理图片生成任务
   */
  private async handleTaskGeneration(
    jobData: TaskGenerationJob,
  ): Promise<void> {
    const { taskId, userId, plan, gender, style, fileKey, idempotencyKey } =
      jobData;

    console.log(
      `📸 Processing task generation: ${taskId} (${plan}, ${gender}, ${style})`,
    );
  ...
  private async simulateProgress(taskId: string): Promise<void> {
    ...
    for (let i = 1; i < timeline.length; i++) {
      ...
      // 模拟处理时间
      await this.sleep(20000); // 2秒间隔，模拟真实处理时间
